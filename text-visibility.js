...
createUnderwaterParticles() {
  ...
  loadFont() {
    ...
    // Modify createFrameText to ensure robust text creation
    createFrameText(text, frame, width, height) {
      ...
      try {
        ...
        // Create two text meshes - one for front, one for back
        const frontTextMesh = new THREE.Mesh(textGeometry.clone(), textMaterial.clone());
        const backTextMesh = new THREE.Mesh(textGeometry.clone(), textMaterial.clone());
  
        // Store additional userData for both meshes
        const sharedUserData = {
          index: frame.userData.index,
          initialOpacity: 0.7,
          fadeSpeed: 0.03 + Math.random() * 0.02
        };
  
        frontTextMesh.userData = { ...sharedUserData, side: 'front' };
        backTextMesh.userData = { ...sharedUserData, side: 'back' };
  
        

        return { frontText: frontTextMesh, backText: backTextMesh };
      } catch (error) {
...
const height = (closestPoint.frameHeight || 1) * 2.5; // Multiplied by 1.5 for 50% larger
    ...
    this.loadFont().then(() => {
      if (this.videoFrames.length > 0) {
        this.frameTexts = this.videoFrames.map((frame, index) => {
          try {
            const textPair = this.createFrameText(
            ...
            if (textPair) {
              this.scene.add(textPair.frontText);
              this.scene.add(textPair.backText);
              return textPair;
            }
          } catch (error) {
            console.error(`Error creating text for frame ${index}:`, error);
          }
        }).filter(Boolean); // Remove any null entries

        console.log(`Created ${this.frameTexts.length} frame text pairs`);
      } else {
        console.warn('No video frames created');
      }
    }).catch(error => {
      console.error('Error in font loading or text creation:', error);
    });
  } catch (error) {
    console.error('Error in createVideoFrames:', error);
  }
}
checkTextVisibility() {
  if (!this.frameTexts || this.frameTexts.length === 0) {
    console.warn('No frame texts available to check');
    return;
  }
  
  console.log(`Total frame texts: ${this.frameTexts.length}`);
  
  this.frameTexts.forEach((textPair, index) => {
    if (!textPair || !textPair.frontText || !textPair.backText) {
      console.warn(`Text pair ${index} is incomplete:`, textPair);
      return;
    }
    
    const frontVisible = textPair.frontText.visible;
    const backVisible = textPair.backText.visible;
    const frontOpacity = textPair.frontText.material ? textPair.frontText.material.opacity : 'N/A';
    const backOpacity = textPair.backText.material ? textPair.backText.material.opacity : 'N/A';
    
    console.log(`Text ${index}: frontVisible=${frontVisible}, frontOpacity=${frontOpacity}, backVisible=${backVisible}, backOpacity=${backOpacity}`);
  });
}
  ...
  updateScene() {
    ...
    this.videoFrames.forEach(frame => {
      ...
      if (Math.abs(targetScale - newScale) < 0.001) {
        ...
        // Modify the frame texts animation with more robust error checking
    if (this.frameTexts && this.frameTexts.length > 0) {
      this.frameTexts.forEach((textPair) => {
        try {
          // Ensure we have both front and back texts
          if (!textPair.frontText || !textPair.backText) {
            console.warn('Incomplete text pair:', textPair);
            return;
          }

          // Ensure userData exists and has an index
          const frontTextData = textPair.frontText.userData || {};
          const backTextData = textPair.backText.userData || {};

          // Find corresponding frame
          const correspondingFrame = this.videoFrames.find(
            frame => frame.userData && frame.userData.index === frontTextData.index
          );

          if (!correspondingFrame) {
            console.warn('No corresponding frame found for text:', frontTextData);
            return;
          }

          // Update position and rotation
          textPair.frontText.position.copy(correspondingFrame.position);
          textPair.backText.position.copy(correspondingFrame.position);

          // Slight z-offset
          textPair.frontText.position.z += 0.1;
          textPair.backText.position.z -= 0.1;

          // Copy frame's orientation
          textPair.frontText.quaternion.copy(correspondingFrame.quaternion);
          textPair.backText.quaternion.copy(correspondingFrame.quaternion);

          // Rotate back text 180 degrees around Y
          textPair.backText.rotateY(Math.PI);

          // Fade and visibility logic
          const isBehindSpine = correspondingFrame.position.z > 3;
          
          if (isBehindSpine) {
            correspondingFrame.renderOrder = -1;
            correspondingFrame.material.opacity = 0.4;

            textPair.frontText.visible = false;
            textPair.backText.visible = true;
            
            if (textPair.backText.material) {
              textPair.backText.material.opacity = 0.5;
            }
            if (textPair.frontText.material) {
              textPair.frontText.material.opacity = 0;
            }
          } else {
            correspondingFrame.renderOrder = 1;
            correspondingFrame.material.opacity = 0.5;

            textPair.frontText.visible = true;
            textPair.backText.visible = false;
            
            if (textPair.frontText.material) {
              textPair.frontText.material.opacity = 0.7;
            }
            if (textPair.backText.material) {
              textPair.backText.material.opacity = 0;
            }
          }

        } catch (error) {
          console.error('Error processing frame text:', error);
        }
      });
    } else {
      console.warn('No frame texts available');
    }
  ...
  this.videoFrames.forEach((frame, index) => {
    ...
    const correspondingTextPair = this.frameTexts.find(
      textPair => textPair.frontText.userData.index === frame.userData.index
    );

    if (correspondingTextPair) {
      const { frontText, backText } = correspondingTextPair;

      // Position both texts with the frame
      frontText.position.copy(frame.position);
      backText.position.copy(frame.position);

      // Slight z-offset
      frontText.position.z += 0.06;
      backText.position.z -= 0.06;

      // Copy frame's orientation
      frontText.quaternion.copy(frame.quaternion);
      backText.quaternion.copy(frame.quaternion);
      ...
      if (isBehindSpine) {
        // When behind spine, hide front text, show back text
        frontText.visible = false;
        backText.visible = true;
        
        // Ensure back text is visible with some opacity
        if (backText.material) {
          backText.material.opacity = 0.7;
        }
      } else {
        // When in front, show front text
        frontText.visible = true;
        backText.visible = false;
        
        // Ensure front text is visible with some opacity
        if (frontText.material) {
          frontText.material.opacity = 0.7;
        }
      }
    }
  });
